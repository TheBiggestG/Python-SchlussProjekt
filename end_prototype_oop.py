...
class Encoder:
    def __init__(self, key, message, option): #when encoder is called, the message will be encoded with the key and will be given out as a Ascii list
            ...
            for i in message:
                converted [i] = ord(chr(message[i]))
            for i in converted:
                encodedList [i] = converted[i] ^ key
...
class Decoder:
    def __init__(self, key, message, option):
        ...
        converted =[]
        decodedList = []
        messageList = string.split(message)
        for i in messageList:
            for x in messageList[i]:
                converted [i][x] = ord(chr(messageList[i][x]))
        for i in converted:
            for x in converted[i]:
                decodedList [i][x] = converted[i][x] ^ key
